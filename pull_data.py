...
def pull_data(std_date, date_start, N):
    ...
    df = pd.read_csv('./data/{}_total_stock.csv'.format(std_date),
                     encoding='euc-kr')  # 한국거래소 > 정보데이터 시스템 > 전종목 시세
    ...
    def earning_rate(stock_price: list, start_date, end_date):
        ...
        possible_start_date = stock_price[stock_price.index >=
                                          start_date].index[0]
        possible_end_date = stock_price[stock_price.index <=
                                        end_date].index[-1]
    ...
    for date in date_list_for_PER:
        ...
        df_PER_dic[date] = pd.read_csv(
            './data/{}_PER_PBR.csv'.format(date), encoding='euc-kr')
...
if __name__ == "__main__":
    ...
    data.to_csv(f'stock_data_{std_date}_{N}.csv',
                encoding='utf-8')  # 이게 더 좋은듯?
